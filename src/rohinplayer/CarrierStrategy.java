...
//        }
    ...
    static void scanWells(RobotController rc) throws GameActionException {
        WellInfo[] wells = rc.senseNearbyWells();
        WellInfo targetWell = null;
        int smallestDistance = 100;
        // if(wells.length > 0) wellLoc = wells[0].getMapLocation();
        // find closest well
        if (wells.length > 0) {
            for (WellInfo well : wells) {
                int wellDistance = well.getMapLocation().distanceSquaredTo(rc.getLocation());
                if (wellDistance < smallestDistance) {
                    smallestDistance = wellDistance;
                    targetWell = well;
                }
            }
            wellLoc = targetWell.getMapLocation();
        }
    }
    ...
    static void scanIslands(RobotController rc) throws GameActionException {
        ...
        for(int id : ids) {
            if(rc.senseTeamOccupyingIsland(id) == Team.NEUTRAL) {
                ...
                int smallestDistance = 100;
                MapLocation targetIslandLoc = null;
                if(locs.length > 0) {
//                    islandLoc = locs[0];
//                    break;
                    // find closest island
                    for (MapLocation loc : locs) {
                        int islandDistance = loc.distanceSquaredTo(rc.getLocation());
                        if (islandDistance < smallestDistance) {
                            smallestDistance = islandDistance;
                            targetIslandLoc = loc;
                        }
                    }
                    islandLoc = targetIslandLoc;
                }
            }
        }
    }
